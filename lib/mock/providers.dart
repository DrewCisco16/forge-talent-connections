...
final FutureProvider<List<AvatarOption>> avatarsProvider =
    FutureProvider<List<AvatarOption>>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadAvatars(),
    );

final FutureProvider<List<ServiceBranch>> branchesProvider =
    FutureProvider<List<ServiceBranch>>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadBranches(),
    );

final FutureProvider<ElevatorPitch> elevatorPitchProvider =
    FutureProvider<ElevatorPitch>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadElevatorPitch(),
    );

final FutureProvider<List<Opportunity>> opportunitiesProvider =
    FutureProvider<List<Opportunity>>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadOpportunities(),
    );
...
final FutureProviderFamily<Opportunity, String> opportunityProvider =
    FutureProvider.family<Opportunity, String>(
      (Ref ref, String id) =>
...
final FutureProviderFamily<MatchSuggestion, String> matchSuggestionProvider =
    FutureProvider.family<MatchSuggestion, String>(
      (Ref ref, String id) =>
          ref.watch(forgeRepositoryProvider).loadMatchSuggestion(id),
    );

final FutureProvider<List<Credential>> credentialsProvider =
    FutureProvider<List<Credential>>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadCredentials(),
    );

final FutureProvider<TrustWalletSummary> walletSummaryProvider =
    FutureProvider<TrustWalletSummary>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadWalletSummary(),
    );

final FutureProvider<List<Vouch>> vouchesProvider = FutureProvider<List<Vouch>>(
  (Ref ref) => ref.watch(forgeRepositoryProvider).loadVouches(),
);

final FutureProvider<ProjectSpace> projectSpaceProvider =
    FutureProvider<ProjectSpace>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadProjectSpace(),
    );

final FutureProvider<List<Story>> storiesProvider = FutureProvider<List<Story>>(
  (Ref ref) => ref.watch(forgeRepositoryProvider).loadStories(),
);

final FutureProvider<List<FeedPost>> feedProvider =
    FutureProvider<List<FeedPost>>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadFeed(),
    );

final FutureProvider<ChatThread> chatThreadProvider =
    FutureProvider<ChatThread>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadThread(),
    );

final FutureProvider<List<AppNotification>> notificationsProvider =
    FutureProvider<List<AppNotification>>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadNotifications(),
    );

final FutureProvider<List<ExportItem>> exportQueueProvider =
    FutureProvider<List<ExportItem>>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadExportQueue(),
    );

final FutureProvider<IntegrityCertificate> certificateProvider =
    FutureProvider<IntegrityCertificate>(
...
final FutureProvider<MembershipStatus> membershipProvider =
    FutureProvider<MembershipStatus>(
...
final FutureProvider<List<VerifiedSkill>> verifiedSkillsProvider =
    FutureProvider<List<VerifiedSkill>>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadVerifiedSkills(),
    );

final FutureProvider<IntegrityStreak> streakProvider =
    FutureProvider<IntegrityStreak>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadStreak(),
    );

final FutureProvider<List<GivenVouch>> givenVouchesProvider =
    FutureProvider<List<GivenVouch>>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadGivenVouches(),
    );

final FutureProvider<List<SystemDecision>> decisionsProvider =
    FutureProvider<List<SystemDecision>>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadDecisions(),
    );

final FutureProvider<PitchStudio> pitchStudioProvider =
    FutureProvider<PitchStudio>(
      (Ref ref) => ref.watch(forgeRepositoryProvider).loadPitchStudio(),
    );
