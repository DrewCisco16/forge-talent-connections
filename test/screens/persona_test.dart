...
void main() {
  ...
  group("demo persona", () {
    test("the persona avatar is drawn from the male set", () {
      final int index = kAvatars
          .indexWhere((AvatarOption a) => a.asset == kDrewAvatar);
      expect(index, greaterThanOrEqualTo(firstMaleIndex),
          reason: "kDrewAvatar must be one of heroes 10-16");
      expect(index, kDrewAvatarIndex,
    ...
    test("the profile served to the app uses that avatar", () async {
      for (final DemoScenario scenario in DemoScenario.values) {
        final UserProfile profile =
            await MockForgeRepository(scenario).loadProfile();
        expect(profile.avatarAsset, kDrewAvatar,
    ...
    test("the persona's own story ring uses that avatar", () async {
      final List<Story> stories =
          await const MockForgeRepository(DemoScenario.verified).loadStories();
    ...
    test("no two people in the cast share a face", () async {
      const MockForgeRepository repo =
          MockForgeRepository(DemoScenario.verified);
      final List<Story> stories = await repo.loadStories();
      final List<String> faces =
          stories.map((Story s) => s.avatar).toList();
