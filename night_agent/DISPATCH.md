...
### 3.7 FINAL
First write `final/kills-all.md` by concatenating every KILLS section in stage order, and `final/metrics-summary.json` from `metrics.jsonl`; then guard: `na_gate.py <run> FINAL`. New CLOSER tab. Send `PROMPTS/P6_final.md` with: ASK verbatim, CLASS, KIND, SUCCESS_CRITERIA, HARD_CONSTRAINTS, MERGED, OPTIONS STANDING, OPEN, CONFLICT, kills-all.md, review.md labelled only R, check-review.md, metrics-summary.json, and the flags so far. Save as `final/DELIVERABLE.md`. Written once. If it exists, the step is done.
