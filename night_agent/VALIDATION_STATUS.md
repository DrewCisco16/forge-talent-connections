# Validation status, Night Agent v11.3.0 (2026-09-10)

## PROVEN BY TEST (executed this session, reproducible with the files in TESTS/)
- Against a frozen, spec-derived corpus built outside the package's own fixture (AUTORESEARCH/holdout_corpus.py: 70 run-folder faults, 20 transitions that must block, 18 that must allow, 15 package assertions), the v11.3 checker and guard score 70 of 70, 20 of 20, 0 false failures, 0 false blocks and 15 of 15. The v11.2 package scored 6 of 70, 1 of 20, 3 false failures on a conforming DIRECT run, 1 false block and 3 of 15 on the same corpus (AUTORESEARCH/baseline.json). Reproduce with `python3 AUTORESEARCH/eval.py`. The corpus author also edited the checker, so this is DEV-grade evidence in the spec's own terms.
- The package is internally consistent on 76 automated checks: six statuses named everywhere they must be; eight nevers; every prompt DISPATCH references exists; deliverable sections match between P6 and SCHEMA; no em-dash; prompts fit 72 columns; regression probes for I1, I2, I3, I4, I7, I8, I9, I10, I11 pass; guards present and invoked; HOLDOUT split, paired design, claim ids, and the adaptation label present.
...
## NOT YET TESTED
- The three WORKBOOKS PDFs against v11.3: they were built from v11.2 prompts and P4 changed; the Reference Manual is STALE for P4 until rebuilt.
