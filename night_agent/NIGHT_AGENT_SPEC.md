...
Version: 11.4.0 (2026-09-26). Supersedes Night Agent v10, v11.0.0, v11.1.0, v11.2.0 and v11.3.0 (Night_Agent_v10.pdf, the v10 workbooks, and the v10 night-agent skill file).

Change log 11.4.0 (SDK runtime): the package gains a second implementation of Dispatch, the deterministic runtime AGENT/run_night.py, which drives every seat through the Claude Agent SDK as a fresh session per send and performs CHECK with its own code (DISPATCH Section 9). Spec-debt item I7 is amended: the API route is now specified, and a seat that can be neither a browser tab nor an SDK seat is UNAVAILABLE. Seats registered by the runtime carry an sdk:// url (Section 2); a slot the runtime closed at MAX_WAIT is recorded as an aborted capture that never counts and does not block the one retry (Sections 7, 15); Dispatch writes every filled packet as packet.md beside the replies so the leak test and the neutrality scan are auditable (Sections 4.1, 16); P6 headings use one space and the closer receives the classification Dispatch computed under Section 17; a fake-seat mode exists for rehearsal and CI and its output is never a result. No real night ran under the runtime; nothing here claims the night got better (NO_AUTORESEARCH_SUPERIORITY_CLAIM stands).

Change log 11.3.0 (autoresearch pass on the package itself, AUTORESEARCH/): the checker and the guards were iterated one rule at a time against a frozen, spec-derived corpus of 70 run-folder faults and 20 transition cases built outside the package's own fixture, from 6 of 70 faults caught and 1 of 20 transitions blocked at baseline to 70 of 70 and 20 of 20, with zero false failures on conforming runs and zero false blocks; the Section 15 rows for G-2 to G-9 state the added clauses and G-11 gains the row Section 19 referred to; P4 MERGE mode writes OPTIONS STANDING (Section 4.3); STRUCTURAL kill vocabulary is gone from Section 8, SCHEMA, P4 and the morning template; SCHEMA gains the DIRECT state, the seat fresh and prior_history fields and the experiment direction field; GATE_DEFAULTED is listed in Section 10. No real night ran; nothing here claims the night got better (NO_AUTORESEARCH_SUPERIORITY_CLAIM stands).
...
- Dispatch: the agent that executes DISPATCH.md: either the browser-driving agent (Claude in Chrome) or the deterministic SDK runtime AGENT/run_night.py, which drives seats through the Claude Agent SDK (DISPATCH Section 9). Dispatch is not a thinker and never decides truth.
- Seat: a model window (browser runtime) or a model configuration addressed by an sdk:// url (SDK runtime) used for one role. Seats are addressed by registry id, never by tab position.
...
Registry (registry.json) maps seat id to URL (a conversation URL for a browser seat; `sdk://<provider>/<model>/<seat id>` for an SDK seat, `fake://fake/<seat id>` for a fake seat in rehearsal), role, model label, runtime, and READY status. Model assignments are configuration, not spec. The v10 assignments (G1 GPT-5.6 Sol, G2 Gemini 3.1 Pro, G3 Grok, G4 Mistral Magistral, CLOSER Claude Fable 5.1 high, REVIEWER GPT-6 Astra xhigh, VERIFIER new chat in the operator's Claude Project) are INHERITED, UNVERIFIED defaults and may be changed per run without changing this spec.
...
- VERIFIER. A new chat in the operator's Claude Project, or in the SDK runtime a fresh session that receives the project documents and nothing else. Sees only the material result and its claims. Compares against private ground truth (project documents).
- EXECUTOR (optional). A tool-capable seat that can run commands, tests, or scripts (Claude Code, a Python tab, a CI job, or in the SDK runtime a seat whose tools are confined to the experiment sandbox by its working directory and a tool hook). Required for EXPERIMENT class. Dispatch may act as EXECUTOR only for arithmetic, link resolution, and document reads.
...
- Allowed check domains: the cited publisher, doi.org, crossref.org, openalex.org, pubmed.ncbi.nlm.nih.gov, any .gov, the operator's project documents, and the EXECUTOR's own sandbox. Nothing else. In the SDK runtime the check is performed by the runtime's own resolver code against the same list, never by a model.
...
### 4.1 GENERATE
- Every generator gets a brand-new empty tab (browser runtime) or a brand-new session (SDK runtime) and P2_generate.md with the ask, kind, constraints, and recency rule. Dispatch writes the filled packet once as packet.md in the stage folder; phrases that came from the packet are not leaks. It sees no other seat's words, name, or output (the wall). Telling it that it is one of several independent reviewers is permitted and required; showing it anything another seat produced is a leak (see I1).
...
### 4.3 CLOSE
The closer (P4_close.md) in MODE=LIST after GENERATE builds the numbered option list, merging only true duplicates, carrying each option's falsification conditions. In MODE=MERGE after every operator stage it writes MERGED (PASSED claims only, each with provenance and claim ids), KILLS (option, the FAILED claim or the explicit hard constraint that killed it; every kill is EARNED), DEPRIORITIZED (options the reviewers found less persuasive, with the reason; they remain STANDING and are still attacked by later operators and shown to the reviewer and in the final), OPEN, CONFLICT (disagreements the check could not settle; never resolved by picking a side), and METRICS. Options die only by executed falsification or an explicit constraint (pilot protocol item 7, Astra 13). Every MERGE close also writes OPTIONS STANDING: the numbered options that still stand after the stage, deprioritized ones included, numbered as in the LIST close; Dispatch pastes it, unchanged, into the next operator packet, the review package and the final. Without it Dispatch would have to decide what stands, and Dispatch never decides. A cost-better option is never dominated by a quality-better one; both stand until weights or feasibility constraints are frozen and declared.
...
- Capture records (pilot protocol item 6, Astra 12): every saved reply has one line in capture.jsonl: dispatch_id, file, completion_signal_observed (yes or no), start_boundary, end_boundary, char_count, byte_hash, observed_identity. The body is captured from observed message boundaries, never by subtracting the prompt length from the page text. An incomplete capture is saved as partial with its slot preserved and is not counted toward MIN_CREW. No new dispatch to a seat while its previous slot is uncertain or still generating. A slot that Dispatch itself terminated (MAX_WAIT reached, or a dispatch found without a capture on resume) is recorded as a capture with partial = yes and aborted = yes: it is closed, not uncertain, it never counts toward MIN_CREW, no seat file may cite it, and it does not block the one retry. A stage is fully captured only when every counted seat has completion_signal_observed = yes.
- Runtime mapping (SDK runtime, DISPATCH Section 9): a tab is a new SDK session per send; the registry url is the seat's sdk:// url and is what every dispatch record observes; tab_id is the session id the runtime assigned before the send; the capture boundaries are the first assistant message and the result message; completion_signal_observed is a result message without an error; an authentication or model error from the SDK is FAILED for the stage, never repaired by the runtime.
...
- Per-run metrics (metrics-summary.json): correctness against the known outcome, planted faults caught over planted faults total, unsupported claims in the final, contradictions with supplied documents, EARNED kills, DEPRIORITIZED options, review HITS that passed, verifier CONTRADICTIONS, model calls, elapsed time, tokens where reported, and the operator's morning usefulness rating (1 to 5).
...
Dispatch never changes a system setting. Power, sleep, update, and browser settings are operator pre-work (workbook SETUP). Dispatch may only read them (for example powercfg /requests) to confirm the run can start; in the SDK runtime they are not applicable (see I7).
...
| I7 | Power commands "Dispatch detects the OS and runs the matching block" vs "never change a system setting"; API route suggested with no procedure | Rail violation or undefined path | Dispatch never changes settings; it may read. Machine setup is operator pre-work. From 11.4.0 the API route is specified: the SDK runtime (DISPATCH 9, AGENT/) drives seats through the Claude Agent SDK, each registered under an sdk:// url, and performs CHECK with its own code; a seat that can be neither a browser tab nor an SDK seat is UNAVAILABLE | Rails outrank convenience; unspecified procedures are not procedures | SPEC 0, 2, 7, 11; DISPATCH 0, 2.1 and 9; SCHEMA seat_record, capture_record; AGENT/; workbook SETUP; MIGRATION |
...
NIGHT_AGENT_SPEC.md > DISPATCH.md > PROMPTS/ > SCHEMA.json > TESTS/ > workbooks > any skill file or runtime (AGENT/). A skill file is generated from DISPATCH.md and is regenerated whenever DISPATCH.md changes. The runtime executes DISPATCH.md; a runtime that disagrees with DISPATCH.md is defective.
...
| G-6 | to REVIEW | last close.md missing, the REVIEWER seat not READY, reviewer handshake count not exactly one, the handshake logged after the first GENERATE send, or any reviewer send logged after the handshake |
| G-7 | to FINAL | final/DELIVERABLE.md already exists, kills-all.md or metrics-summary.json missing or metrics-summary.json not valid JSON, kills-all.md missing any KILLS entry of any close, or neither (review.md and check-review.md) nor the NO_OUTSIDE_REVIEW flag present |
| G-8 | to VERIFY | DELIVERABLE.md missing, no READY VERIFIER seat, verifier.md already exists, or section 14 of DELIVERABLE.md already has content |
...
- Packet neutrality (Astra 07): every packet sent to a generator or the reviewer is built from an allowlist: observations, unresolved propositions, dissent with provenance, options standing and deprioritized with reasons. It never contains a winner label, model-role praise, a holdout answer, a previous final verdict, or an unsupported count assertion. Dispatch runs a lexical scan for those before every send and records it (the SDK runtime writes every packet it sent as packet.md beside the replies, so the scan is auditable); the scan is a floor, not proof, because paraphrase defeats it, so first answers are preserved before any peer content is shared.
